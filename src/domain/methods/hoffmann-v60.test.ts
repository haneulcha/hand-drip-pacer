...
describe('Hoffmann V60', () => {
  ...
  describe('invariants', () => {
    it.each([10, 15, 20, 25, 30])(
      ...
      (coffeeG) => {
        for (const taste of allCombos) {
          const r = hoffmannV60.compute(baseInput({ coffee: g(coffeeG), taste }))
    ...
    it('totalTimeSec fixed at 210 (3:30)', () => {
      for (const taste of allCombos) {
        expect(hoffmannV60.compute(baseInput({ taste })).totalTimeSec).toBe(210)
      }
    })
  ...
  describe('grind by taste (boolean OR: at most ±1 step, opposing cancels)', () => {
    ...
    it.each(cases)(
      ...
      ({ sweetness, strength, expected }) => {
        expect(
          hoffmannV60.compute(baseInput({ taste: { sweetness, strength } })).grindHint,
        ).toBe(expected)
      },
    )
  })
  ...
  describe('metadata', () => {
    it('temperature by roast: light=96, medium=93, dark=90', () => {
      expect(hoffmannV60.compute(baseInput({ roast: 'light' })).temperature).toBe(96)
      expect(hoffmannV60.compute(baseInput({ roast: 'medium' })).temperature).toBe(93)
      expect(hoffmannV60.compute(baseInput({ roast: 'dark' })).temperature).toBe(90)
    })

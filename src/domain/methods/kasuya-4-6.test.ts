...
describe('Kasuya 4:6', () => {
  describe('spec snapshots', () => {
    ...
    it('20g / sweet / strong — classic 5-pour', () => {
      expect(
        kasuya46.compute(baseInput({ taste: { sweetness: 'sweet', strength: 'strong' } })),
    ...
    it('20g / bright / light — minimal 3-pour', () => {
      expect(
        kasuya46.compute(baseInput({ taste: { sweetness: 'bright', strength: 'light' } })),
      ).toMatchInlineSnapshot(`
        {
          "coffee": 20,
          "dripper": "v60",
  ...
  describe('invariants', () => {
    ...
    it('first pour starts at 0s, subsequent pours at 45s intervals', () => {
      const r = kasuya46.compute(baseInput({ taste: { sweetness: 'sweet', strength: 'strong' } }))
    ...
    it('pour count by strength: light=3, medium=4, strong=5', () => {
      expect(
        kasuya46.compute(baseInput({ taste: { sweetness: 'balanced', strength: 'light' } })).pours
          .length,
      ).toBe(3)
      expect(
        kasuya46.compute(baseInput({ taste: { sweetness: 'balanced', strength: 'medium' } })).pours
          .length,
      ).toBe(4)
      expect(
        kasuya46.compute(baseInput({ taste: { sweetness: 'balanced', strength: 'strong' } })).pours
          .length,
      ).toBe(5)
    })
  })
  ...
  describe('metadata', () => {
    it('temperature by roast: light=93, medium=90, dark=87', () => {
      expect(kasuya46.compute(baseInput({ roast: 'light' })).temperature).toBe(93)
      expect(kasuya46.compute(baseInput({ roast: 'medium' })).temperature).toBe(90)
      expect(kasuya46.compute(baseInput({ roast: 'dark' })).temperature).toBe(87)
    })

...
const allCombos = (['sweet', 'balanced', 'bright'] as const).flatMap((sweetness) =>
  (['light', 'medium', 'strong'] as const).map((strength) => ({ sweetness, strength })),
)
...
describe('Kalita Wave pulse', () => {
  describe('spec snapshots', () => {
    ...
    it('20g / sweet / strong — longer bloom + heavy middle pulses', () => {
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'sweet', strength: 'strong' } })),
    ...
    it('20g / bright / light — short bloom + 4 pours', () => {
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'bright', strength: 'light' } })),
      ).toMatchInlineSnapshot(`
        {
          "coffee": 20,
          "dripper": "kalita_wave",
  ...
  describe('invariants', () => {
    it.each([10, 15, 20, 25, 30])(
      ...
      (coffeeG) => {
        for (const taste of allCombos) {
          const r = kalitaPulse.compute(baseInput({ coffee: g(coffeeG), taste }))
    ...
    it('pour count: light=4, medium=5, strong=5', () => {
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'balanced', strength: 'light' } })).pours,
      ).toHaveLength(4)
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'balanced', strength: 'medium' } })).pours,
      ).toHaveLength(5)
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'balanced', strength: 'strong' } })).pours,
      ).toHaveLength(5)
    })
    ...
    it('first pulse starts at bloom duration (30/45/60s)', () => {
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'sweet', strength: 'medium' } }))
          .pours[1]!.atSec,
      ).toBe(60)
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'balanced', strength: 'medium' } }))
          .pours[1]!.atSec,
      ).toBe(45)
      expect(
        kalitaPulse.compute(baseInput({ taste: { sweetness: 'bright', strength: 'medium' } }))
          .pours[1]!.atSec,
      ).toBe(30)
    })
    ...
    it('strong differs from medium by pour amounts (weights [3,5,5,3] vs [3,4,4,3])', () => {
      const med = kalitaPulse
        .compute(baseInput({ taste: { sweetness: 'balanced', strength: 'medium' } }))
        .pours.map((p) => p.pourAmount)
      const strong = kalitaPulse
        .compute(baseInput({ taste: { sweetness: 'balanced', strength: 'strong' } }))
        .pours.map((p) => p.pourAmount)
  ...
  describe('metadata', () => {
    it('temperature by roast: light=96, medium=93, dark=90', () => {
      expect(kalitaPulse.compute(baseInput({ roast: 'light' })).temperature).toBe(96)
      expect(kalitaPulse.compute(baseInput({ roast: 'medium' })).temperature).toBe(93)
      expect(kalitaPulse.compute(baseInput({ roast: 'dark' })).temperature).toBe(90)
    })

    it('grind is always medium (taste adjusts bloom time / pour count instead)', () => {
      for (const taste of allCombos) {
        expect(kalitaPulse.compute(baseInput({ taste })).grindHint).toBe('medium')
      }
    })
